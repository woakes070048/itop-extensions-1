			 	...
			 	else
			 	{
					$aArgs = array('this' => $oObj->ToArgs());
			 		...
			 		else
			 		{
				 		...
				 		{
				 			if (array_key_exists($value, $aValues)) {
				 				$bFound = true;
				 				$oObj->Set($sAttCode, $value);
				 			}
				 		...
				 		if (!$bFound)
				 		{
				 			$this->Trace("Warning: cannot set the value '$value' for the field $sAttCode of the ticket. '$value' is not a valid value for $sAttCode. Allowed values: '".var_dump($aValues)."'");
				 		}
			 		}
				}
			}
